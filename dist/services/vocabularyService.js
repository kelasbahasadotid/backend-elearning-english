...
exports.recordVocabularyItem = recordVocabularyItem;
exports.extractVocabFromQuiz = extractVocabFromQuiz;
exports.extractVocabFromSpeaking = extractVocabFromSpeaking;
exports.getStudentVocabularyRoom = getStudentVocabularyRoom;
exports.getVocabularyItemDetails = getVocabularyItemDetails;
exports.checkWordDuplicate = checkWordDuplicate;
exports.updateVocabularyItem = updateVocabularyItem;
exports.deleteVocabularyItem = deleteVocabularyItem;
 ...
 */
async function recordVocabularyItem(input) {
    const { userId, term, translation = null, phoneticIpa = null, contextSentence = null, sourceType, sourceId = null, sourceTitle = null, notes = null, masteryLevel = 'NEW' } = input;
    ...
    if (!finalIpa && cleanTerm.split(/\s+/).length <= 4) {
        finalIpa = (0, speechEngine_1.getWordIPA)(cleanTerm);
    }
    const conn = await db_1.default.getConnection();
    try {
        // 1. Check if term already exists in this student's room
        const [existingRows] = await conn.query('SELECT * FROM student_vocabularies WHERE user_id = ? AND normalized_term = ?', [userId, normalized]);
        if (existingRows && existingRows.length > 0) {
            ...
            await conn.query(`UPDATE student_vocabularies 
         ...
         WHERE id = ?`, [
            ...
            await conn.query(`INSERT INTO student_vocabulary_encounters 
         (vocabulary_id, user_id, source_type, source_id, source_title, context_sentence, encounter_number)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [existing.id, userId, sourceType, sourceId || null, sourceTitle || null, contextSentence || null, newEncounterCount]);
            const [updatedRows] = await conn.query('SELECT * FROM student_vocabularies WHERE id = ?', [existing.id]);
            return {
                item: updatedRows[0],
                isDuplicate: true,
                duplicateInfo: {
            ...
            // NEW VOCABULARY ITEM
            const [insertResult] = await conn.query(`INSERT INTO student_vocabularies 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, ?, ?)`, [
            ...
            await conn.query(`INSERT INTO student_vocabulary_encounters 
         (vocabulary_id, user_id, source_type, source_id, source_title, context_sentence, encounter_number)
         VALUES (?, ?, ?, ?, ?, ?, 1)`, [newId, userId, sourceType, sourceId || null, sourceTitle || null, contextSentence || null]);
            const [newRows] = await conn.query('SELECT * FROM student_vocabularies WHERE id = ?', [newId]);
            return {
                item: newRows[0],
                isDuplicate: false,
                duplicateInfo: null
 ...
 */
async function extractVocabFromQuiz(userId, assessmentId, quizTitle, questions, userAnswers) {
    if (!questions || !Array.isArray(questions))
        return [];
    const recordedItems = [];
    for (const q of questions) {
        const qId = q.id;
        const userAns = Array.isArray(userAnswers) ? userAnswers.find((a) => a.questionId === qId) : userAnswers?.[qId];
        ...
        // MULTIPLE CHOICE (Type 1), MULTIPLE ANSWER (Type 2), TRUE/FALSE (Type 3)
            ...
            const chosenOpt = qOptions.find((o) => o.id === Number(selectedOptionId));
 ...
 */
async function extractVocabFromSpeaking(userId, promptId, promptTitle, promptText, transcription, wordAnalysis) {
    const recordedItems = [];
    ...
    if (Array.isArray(wordAnalysis) && wordAnalysis.length > 0) {
        for (const w of wordAnalysis) {
            ...
            if (word && word.length >= 3) {
                const res = await recordVocabularyItem({
                    userId,
                    term: word,
                    phoneticIpa: w.ipa || (0, speechEngine_1.getWordIPA)(word),
        ...
        for (const uw of uniqueWords.slice(0, 10)) { // limit up to 10 key words per prompt
            const res = await recordVocabularyItem({
                userId,
                term: uw,
                phoneticIpa: (0, speechEngine_1.getWordIPA)(uw),
 ...
 */
async function getStudentVocabularyRoom(userId, options) {
    const { search = '', sourceType, masteryLevel, onlyDuplicates = false, sortBy = 'recent', page = 1, limit = 20 } = options;
    ...
    const [countRows] = await db_1.default.query(`SELECT COUNT(*) as total FROM student_vocabularies WHERE ${whereSql}`, params);
    const total = countRows[0]?.total || 0;
    // Fetch paginated data
    const [items] = await db_1.default.query(`SELECT * FROM student_vocabularies WHERE ${whereSql} ORDER BY ${orderSql} LIMIT ? OFFSET ?`, [...params, Number(limit), Number(offset)]);
    // Room Summary Statistics
    const [statsRows] = await db_1.default.query(`
    SELECT 
 ...
 */
async function getVocabularyItemDetails(userId, vocabId) {
    const [vocabRows] = await db_1.default.query('SELECT * FROM student_vocabularies WHERE id = ? AND user_id = ?', [vocabId, userId]);
    if (!vocabRows || vocabRows.length === 0) {
        throw new Error('Kata kosakata tidak ditemukan');
    }
    const [encounters] = await db_1.default.query('SELECT * FROM student_vocabulary_encounters WHERE vocabulary_id = ? AND user_id = ? ORDER BY encounter_number ASC', [vocabId, userId]);
 ...
 */
async function checkWordDuplicate(userId, term) {
    const normalized = normalizeTerm(term);
    if (!normalized) {
        return { isDuplicate: false, message: 'Kata tidak valid' };
    }
    const [rows] = await db_1.default.query('SELECT * FROM student_vocabularies WHERE user_id = ? AND normalized_term = ?', [userId, normalized]);
    if (rows && rows.length > 0) {
        const item = rows[0];
        const [encounters] = await db_1.default.query('SELECT * FROM student_vocabulary_encounters WHERE vocabulary_id = ? ORDER BY encounter_number DESC LIMIT 5', [item.id]);
        return {
            isDuplicate: true,
            term: item.term,
 ...
 */
async function updateVocabularyItem(userId, vocabId, data) {
    const updates = [];
    const values = [];
    ...
    await db_1.default.query(`UPDATE student_vocabularies SET ${updates.join(', ')} WHERE id = ? AND user_id = ?`, values);
    const [rows] = await db_1.default.query('SELECT * FROM student_vocabularies WHERE id = ? AND user_id = ?', [vocabId, userId]);
    return rows[0];
}
/**
 * Delete a vocabulary item from student's room
 */
async function deleteVocabularyItem(userId, vocabId) {
    const [res] = await db_1.default.query('DELETE FROM student_vocabularies WHERE id = ? AND user_id = ?', [vocabId, userId]);
