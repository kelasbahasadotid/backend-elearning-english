...
async function seed() {
    ...
    try {
        ...
        await connection.query(`INSERT INTO course_categories (id, parent_id, name, slug, description, sort_order, status) 
       VALUES 
       ...
       (6, NULL, 'Conversation', 'conversation', 'Interactive dialogue practice and communication scenarios.', 6, 'ACTIVE')`);
        ...
        await connection.query(`INSERT INTO course_levels (id, code, name, description, sort_order, status) 
       VALUES (1, 'A1', 'Beginner', 'Basic daily expressions.', 1, 'ACTIVE')`);
        ...
        await connection.query(`INSERT IGNORE INTO roles (id, code, name, status) 
       VALUES (5, 'CONTENT_MANAGER', 'Content Manager', 'ACTIVE')`);
        const superAdminHash = await bcryptjs_1.default.hash('SuperAdmin123', 10);
        const adminPassHash = await bcryptjs_1.default.hash('Admin123', 10);
        const tutorPassHash = await bcryptjs_1.default.hash('Tutor123', 10);
        const studentPassHash = await bcryptjs_1.default.hash('Student123', 10);
        const contentPassHash = await bcryptjs_1.default.hash('ContentManager123', 10);
        await connection.query(`INSERT INTO users (id, role_id, full_name, email, username, password, status) VALUES 
       ...
       (6, 5, 'Content Manager Global English', 'contentmanager@globalenglish.com', 'contentmanager', ?, 'ACTIVE')`, [superAdminHash, adminPassHash, tutorPassHash, studentPassHash, studentPassHash, contentPassHash]);
        ...
        await connection.query(`INSERT INTO courses (id, category_id, level_id, created_by, code, title, slug, short_description, description, cefr_level, price, discount_price, certificate_enabled, speaking_ai_enabled, status) 
       ...
       'A1', 250000.00, 199000.00, 1, 1, 'PUBLISHED')`);
        ...
        await connection.query(`INSERT INTO course_versions (id, course_id, version, is_current, published_at, created_by) 
       VALUES (1, 1, '1.0', 1, NOW(), 1)`);
        // 7. Seed Modules
        console.log('Seeding modules...');
        await connection.query(`INSERT INTO modules (id, course_version_id, title, description, module_order, estimated_minutes, status) 
       VALUES (1, 1, 'Present Tense Structures', 'Present continuous and basic introduction sentences.', 1, 90, 'PUBLISHED')`);
        ...
        await connection.query(`INSERT INTO lessons (id, module_id, lesson_type, title, slug, lesson_order, duration_minutes, is_preview, is_required, passing_score, xp_reward, status) 
       VALUES 
       ...
       (5, 1, 'EXAM',    'TOEFL Mock Final Exam',                    'toefl-mock-exam',             5, 30, 0, 1, 70, 60, 'PUBLISHED')`);
        ...
        for (const row of lessonContents) {
            await connection.execute(`INSERT INTO lesson_contents (id, lesson_id, content_type, title, description, content_order, is_required, estimated_minutes, status) VALUES (?,?,?,?,?,?,?,?,?)`, row);
        }
        // 10. Seed Assessments (clean — only for lessons 3, 4, 5)
        console.log('Seeding assessments...');
        await connection.query(`INSERT INTO assessments (id, lesson_id, module_id, course_id, assessment_type_id, title, description, instruction, passing_score, duration_minutes, total_question, total_score, status, show_answer) 
       VALUES 
       ...
       (3, 4, 1, 1, 4, 'Pronunciation AI Challenge',
        ...
        await connection.query(`INSERT INTO assessment_sections (id, assessment_id, title, instruction, section_order, duration_minutes, total_question) 
       VALUES 
       ...
       (3, 3, 'Pronunciation Reading Prompts',
        ...
        await connection.query(`INSERT INTO questions (id, assessment_section_id, question_type_id, question_code, title, question_text, point, status) 
       VALUES 
       ...
       (210, 2, 6, 'E10', 'EXAM Order 1 — Complex Sentence',
        ...
        await connection.query(`INSERT INTO question_options (id, question_id, option_label, option_text, is_correct, score, option_order) 
       VALUES 
       ...
       (62, 210, 'ANS', 'scientists have discovered a new species in the ocean', 1, 10.00, 1)`);
        ...
        await connection.query(`INSERT INTO speaking_tests (id, assessment_id, title, instruction) 
       VALUES (1, 3, 'Pronunciation AI Challenge',
        ...
        await connection.query(`INSERT INTO speaking_prompts (id, speaking_test_id, prompt_type, prompt_text, prompt_order) 
       VALUES 
       (1, 1, 'READING',
       ...
       (5, 1, 'READING',
        ...
        await connection.query(`INSERT INTO certificate_templates (id, template_code, template_name, background_image, orientation, paper_size, active) 
       VALUES (1, 'GOLD_LMS', 'Gold Orientation Certificate', 'uploads/templates/gold.png', 'LANDSCAPE', 'A4', 1)`);
        // 16. Seed Banners
        console.log('Seeding banners...');
        await connection.query(`INSERT INTO banners (id, title, image_path, button_text, button_url, active) 
       VALUES (1, 'Welcome to Global English!', 'uploads/banners/welcome.png', 'Get Started', '/#predictor', 1)`);
        // 17. Seed Orders
        console.log('Seeding orders...');
        await connection.query(`INSERT INTO orders (id, order_number, user_id, subtotal, discount, tax, grand_total, payment_status, order_status) 
       VALUES 
       ...
       (2, 'ORD-20260717-002', 5, 250000.00, 51000.00, 0.00, 199000.00, 'PAID', 'SUCCESS')`);
        await connection.query(`INSERT INTO order_items (id, order_id, course_id, price, discount, total) 
       VALUES 
       ...
       (2, 2, 1, 250000.00, 51000.00, 199000.00)`);
        ...
        await connection.query(`INSERT INTO enrollments (id, user_id, course_id, source_id, order_id, enrolled_at, expired_at, access_days, status) 
       VALUES (2, 5, 1, 1, 2, NOW(), DATE_ADD(NOW(), INTERVAL 365 DAY), 365, 'ACTIVE')`);
        await connection.query(`INSERT INTO course_progress (user_id, course_id, enrollment_id, completed_module, total_module, overall_progress, average_score) 
       VALUES (5, 1, 2, 0, 1, 60.00, 85.00)`);
        // Give student Anita Level 2 & 240 XP in user_statistics
        await connection.query(`UPDATE user_statistics SET xp = 240, level = 2 WHERE user_id = 5`);
