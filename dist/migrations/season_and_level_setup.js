...
exports.runSeasonAndLevelMigration = runSeasonAndLevelMigration;
const db_1 = __importDefault(require("../config/db"));
async function runSeasonAndLevelMigration() {
    ...
    `);
        ...
        const [xpCols] = await conn.query('SHOW COLUMNS FROM xp_transactions');
        const hasSeasonId = xpCols.some((c) => c.Field === 'season_id');
        if (!hasSeasonId) {
      ...
      `);
        ...
        const [levelCountRows] = await conn.query('SELECT COUNT(*) as count FROM gamification_levels');
        if (levelCountRows[0].count === 0) {
            ...
            for (const lvl of defaultLevels) {
                await conn.query('INSERT INTO gamification_levels (level_number, name, min_xp, badge_icon, description) VALUES (?, ?, ?, ?, ?)', [lvl.level_number, lvl.name, lvl.min_xp, lvl.badge_icon, lvl.description]);
            }
        }
        // 6. Seed initial active Season 1 if none exists
        const [seasonCountRows] = await conn.query('SELECT COUNT(*) as count FROM leaderboard_seasons');
        if (seasonCountRows[0].count === 0) {
            console.log('Seeding initial active Season 1...');
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1, 0, 0, 0);
            const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59);
            const [res] = await conn.query(`INSERT INTO leaderboard_seasons (title, code, start_date, end_date, reset_schedule_type, status)
         VALUES (?, ?, ?, ?, ?, 'ACTIVE')`, [
