...
exports.getAttachmentFile = exports.getPresentationFile = exports.deleteCourseAnnouncement = exports.createCourseAnnouncement = exports.replyCourseReview = exports.createCourseReview = exports.getCourseDetails = exports.getCourses = exports.getLandingPage = void 0;
const db_1 = __importDefault(require("../config/db"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const getLandingPage = async (req, res) => {
    try {
        // 1. Fetch active banners
        const [banners] = await db_1.default.query('SELECT id, title, image_path, button_text, button_url FROM banners WHERE active = 1 AND (start_date IS NULL OR start_date <= NOW()) AND (end_date IS NULL OR end_date >= NOW())');
        // 2. Fetch popular/featured courses (limit to 6)
        const [courses] = await db_1.default.query(`SELECT c.id, c.code, c.title, c.slug, c.short_description, c.description, c.thumbnail, c.price, c.discount_price, c.cefr_level, c.duration_minutes, cat.name as category_name,
...
const getCourses = async (req, res) => {
    ...
    if (token) {
        try {
            const decoded = jsonwebtoken_1.default.verify(token, process.env.JWT_SECRET || 'supersecret_pronunciation_ai_key');
    ...
    try {
      ...
      ORDER BY c.created_at DESC
    `;
        const [courses] = await db_1.default.query(sql);
        res.json(courses);
    }
    catch (error) {
        res.status(500).json({ error: error.message || 'Internal server error' });
...
const getCourseDetails = async (req, res) => {
    ...
    try {
        ...
        const [courses] = await db_1.default.query(courseQuery, queryParams);
        if (courses.length === 0) {
        ...
        let [versions] = await db_1.default.query('SELECT id FROM course_versions WHERE course_id = ? AND is_current = 1 LIMIT 1', [course.id]);
        if (versions.length === 0) {
            // Auto-provision a default course version if missing
            const [insertResult] = await db_1.default.query('INSERT INTO course_versions (course_id, version, is_current, created_by) VALUES (?, ?, ?, ?)', [course.id, 'v1.0.0', 1, course.created_by || 1]);
            const [reFetch] = await db_1.default.query('SELECT id FROM course_versions WHERE id = ?', [insertResult.insertId]);
            versions = reFetch;
        }
        ...
        const [units] = await db_1.default.query(unitsQuery, [versionId]);
        let modules = [];
        ...
        const [moduleRows] = await db_1.default.query(modulesQuery, [versionId]);
        modules = moduleRows;
        ...
        for (const mod of modules) {
           ...
           ORDER BY l.lesson_order ASC`;
            const [lessonRows] = await db_1.default.query(lessonsQuery, [userId || 0, mod.id]);
            mod.lessons = lessonRows.map((r) => ({
                ...r,
        ...
        let speakingScores = [];
        if (userId) {
            const [qScores] = await db_1.default.query(`SELECT a.id as assessment_id, a.lesson_id, l.title as lesson_title, l.lesson_type, ap.highest_score, ap.total_attempt, ap.passed
         FROM assessments a
         JOIN lessons l ON a.lesson_id = l.id
         JOIN modules m ON l.module_id = m.id
         LEFT JOIN assessment_progress ap ON a.id = ap.assessment_id AND ap.user_id = ?
         WHERE m.course_version_id = ? AND a.status = 'PUBLISHED'`, [userId, versionId]);
            quizScores = qScores;
            const [sScores] = await db_1.default.query(`SELECT st.id as speaking_test_id, a.lesson_id, l.title as lesson_title, l.lesson_type, 
         ...
         GROUP BY st.id, a.lesson_id, l.title, l.lesson_type`, [userId, versionId]);
            speakingScores = sScores;
        }
        // Fetch reviews for this course
        const [reviews] = await db_1.default.query(`SELECT r.id, r.rating, r.review_text, r.created_at, u.full_name as reviewer_name,
              r.reply_text, r.replied_at, u2.full_name as replier_name
       FROM course_reviews r
       JOIN users u ON r.user_id = u.id
       LEFT JOIN users u2 ON r.replied_by = u2.id
       WHERE r.course_id = ?
       ORDER BY r.created_at DESC`, [course.id]);
        // Fetch announcements for this course
        const [announcements] = await db_1.default.query(`SELECT a.id, a.title, a.content, a.created_at, u.full_name as announcer_name
       FROM course_announcements a
       JOIN users u ON a.created_by = u.id
       WHERE a.course_id = ?
       ORDER BY a.created_at DESC`, [course.id]);
        // Check if user is enrolled in this course
        let isEnrolled = false;
        let enrollmentData = null;
        if (userId) {
            const [enrollments] = await db_1.default.query(`SELECT id, user_id, course_id, enrolled_at, expired_at, access_days, status
         FROM enrollments
         WHERE user_id = ? AND course_id = ? AND (status = 'ACTIVE' OR LOWER(status) = 'active') AND (expired_at IS NULL OR expired_at >= NOW())
         ORDER BY id DESC LIMIT 1`, [userId, course.id]);
...
const createCourseReview = async (req, res) => {
    ...
    try {
        const [enrollment] = await db_1.default.query('SELECT id FROM enrollments WHERE user_id = ? AND course_id = ? AND status = "ACTIVE"', [userId, id]);
        if (enrollment.length === 0) {
            res.status(403).json({ error: 'You must be actively enrolled to review this course' });
            return;
        }
        await db_1.default.query(`INSERT INTO course_reviews (course_id, user_id, rating, review_text)
       VALUES (?, ?, ?, ?)`, [id, userId, rating, review_text || '']);
        // Create notifications for all admins
        try {
            const [userRows] = await db_1.default.query('SELECT full_name FROM users WHERE id = ?', [userId]);
            const [courseRows] = await db_1.default.query('SELECT title FROM courses WHERE id = ?', [id]);
            ...
            for (const adminUser of adminRows) {
                await db_1.default.query(`INSERT INTO user_notifications (user_id, title, message) 
           VALUES (?, ?, ?)`, [
                    adminUser.id,
                    'New Student Review',
...
const replyCourseReview = async (req, res) => {
    ...
    try {
        await db_1.default.query(`UPDATE course_reviews 
       SET reply_text = ?, replied_by = ?, replied_at = CURRENT_TIMESTAMP
       WHERE id = ?`, [reply_text, adminId, id]);
        // Create notification for review writer
        try {
            const [reviewRows] = await db_1.default.query(`SELECT r.user_id, c.title 
         FROM course_reviews r
...
exports.replyCourseReview = replyCourseReview;
const createCourseAnnouncement = async (req, res) => {
    ...
    try {
        await db_1.default.query(`INSERT INTO course_announcements (course_id, title, content, created_by)
       VALUES (?, ?, ?, ?)`, [id, title, content, adminId]);
        // Notify all enrolled students (or members if no specific enrollments)
        try {
            const [cRows] = await db_1.default.query('SELECT title FROM courses WHERE id = ?', [id]);
            const courseTitle = cRows[0]?.title || 'Kursus';
            let [students] = await db_1.default.query(`SELECT DISTINCT user_id FROM enrollments WHERE course_id = ?`, [id]);
            if (students.length === 0) {
                [students] = await db_1.default.query(`SELECT id AS user_id FROM users WHERE role_id = 4`);
            }
            for (const student of students) {
                await db_1.default.query(`INSERT INTO user_notifications (user_id, title, message, is_read) VALUES (?, ?, ?, 0)`, [student.user_id, `Pengumuman: ${title}`, `Pengumuman baru di ${courseTitle}: "${title}".`]);
            }
        }
        catch (notifErr) {
...
const deleteCourseAnnouncement = async (req, res) => {
    ...
    try {
        await db_1.default.query('DELETE FROM course_announcements WHERE id = ?', [id]);
...
exports.deleteCourseAnnouncement = deleteCourseAnnouncement;
/**
 ...
 */
const getPresentationFile = async (req, res) => {
    const { contentId } = req.params;
    try {
        const [rows] = await db_1.default.query('SELECT attachments FROM lesson_contents WHERE id = ?', [contentId]);
        ...
        const pptxAtt = attachments.find((a) => a.data && (a.name?.endsWith('.pptx') || a.name?.endsWith('.ppt') || a.name?.endsWith('.ppsx') || a.mime?.includes('presentation'))) || attachments[0];
...
exports.getPresentationFile = getPresentationFile;
/**
