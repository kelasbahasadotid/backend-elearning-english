...
exports.checkSingleQuestion = exports.getAssessmentsByType = exports.getAttemptDetails = exports.getAttemptHistory = exports.submitAttempt = exports.getQuiz = void 0;
...
const getQuiz = async (req, res) => {
    ...
    try {
        ...
        const [assessments] = await db_1.default.query(`SELECT a.*, l.max_attempt 
       FROM assessments a 
       LEFT JOIN lessons l ON a.lesson_id = l.id 
       WHERE a.id = ? AND a.status = "PUBLISHED"`, [id]);
        ...
        if (!isAdminOrTutor) {
            const [enrollments] = await db_1.default.query(`SELECT e.id FROM enrollments e 
         LEFT JOIN course_versions cv ON e.course_id = cv.course_id
         LEFT JOIN modules m ON cv.id = m.course_version_id
         LEFT JOIN lessons l ON m.id = l.module_id
         WHERE (e.course_id = ? OR l.id = ?) AND e.user_id = ? AND (e.status = 'ACTIVE' OR LOWER(e.status) = 'active') AND (e.expired_at IS NULL OR e.expired_at >= NOW())`, [quiz.course_id || 0, quiz.lesson_id || 0, req.user.id]);
            ...
            if (quiz.lesson_id) {
                const lockCheck = await (0, progress_1.checkSequentialLessonLock)(db_1.default, req.user.id, quiz.lesson_id);
        ...
        if (req.user.roleId === 4) {
            const [attempts] = await db_1.default.query('SELECT COUNT(*) as count FROM assessment_attempts WHERE user_id = ? AND assessment_id = ?', [req.user.id, quiz.id]);
            ...
            if (quiz.max_attempt !== null && quiz.max_attempt > 0 && attemptCount >= quiz.max_attempt) {
                ...
                const [lastAttRows] = await db_1.default.query('SELECT * FROM assessment_attempts WHERE user_id = ? AND assessment_id = ? ORDER BY id DESC LIMIT 1', [req.user.id, quiz.id]);
                latestAttempt = lastAttRows[0] || null;
            }
        }
        // 2. Fetch sections
        const [sections] = await db_1.default.query('SELECT id, title, instruction, section_order FROM assessment_sections WHERE assessment_id = ? ORDER BY section_order ASC', [quiz.id]);
        let questions = [];
        if (sections.length > 0) {
            const sectionIds = sections.map(s => s.id);
            // 3. Fetch questions
            const [questionRows] = await db_1.default.query(`SELECT id, assessment_section_id, question_type_id, title, question_text, explanation, point, question_order, shuffle_option, question_image 
         FROM questions 
         ...
         ORDER BY question_order ASC`, sectionIds);
            ...
            const hasShuffleQuestionsMarker = Boolean((quiz.instruction && /\[SHUFFLE_QUESTIONS?\]|<!--\s*SHUFFLE_QUESTIONS?\s*-->|\[RANDOM_QUESTIONS?\]/i.test(quiz.instruction)) ||
            ...
            const hasShuffleOptionsMarker = Boolean((quiz.instruction && /\[SHUFFLE_OPTIONS?\]|<!--\s*SHUFFLE_OPTIONS?\s*-->/i.test(quiz.instruction)) ||
            ...
            if (questions.length > 0) {
                ...
                const [optionRows] = await db_1.default.query(`SELECT id, question_id, option_label, option_text, option_order, option_image${isAttemptExceeded ? ', is_correct' : ''} 
           ...
           ORDER BY option_order ASC`, questionIds);
                ...
                for (const q of questions) {
                    ...
                    if (q.question_type_id === 5) { // MATCHING / PENCOCOKAN
                        ...
                        const matchTargets = rawOpts.map((o) => o.option_text).filter(Boolean);
...
const submitAttempt = async (req, res) => {
    ...
    try {
        ...
        const [assessments] = await connection.query(`SELECT a.id, a.passing_score, a.total_score, a.course_id, a.lesson_id, l.max_attempt 
       FROM assessments a
       LEFT JOIN lessons l ON a.lesson_id = l.id
       WHERE a.id = ?`, [assessmentId]);
        ...
        const isAdminOrTutor = userRoleNum === 1 || userRoleNum === 2 || userRoleNum === 3 || userRoleNum === 5;
        if (!isAdminOrTutor) {
            const [enrollments] = await connection.query(`SELECT e.id FROM enrollments e 
         LEFT JOIN course_versions cv ON e.course_id = cv.course_id
         LEFT JOIN modules m ON cv.id = m.course_version_id
         LEFT JOIN lessons l ON m.id = l.module_id
         WHERE (e.course_id = ? OR l.id = ?) AND e.user_id = ? AND (e.status = 'ACTIVE' OR LOWER(e.status) = 'active') AND (e.expired_at IS NULL OR e.expired_at >= NOW())`, [quiz.course_id || 0, quiz.lesson_id || 0, req.user.id]);
            ...
            if (quiz.lesson_id) {
                const lockCheck = await (0, progress_1.checkSequentialLessonLock)(connection, req.user.id, quiz.lesson_id);
                if (lockCheck.isLocked) {
                    res.status(403).json({
        ...
        // Verify attempt limit if student role
        if (req.user.roleId === 4) {
            const [attempts] = await connection.query('SELECT COUNT(*) as count FROM assessment_attempts WHERE user_id = ? AND assessment_id = ?', [req.user.id, assessmentId]);
            const attemptCount = attempts[0]?.count || 0;
            if (quiz.max_attempt !== null && quiz.max_attempt > 0 && attemptCount >= quiz.max_attempt) {
        ...
        const [sections] = await connection.query('SELECT id FROM assessment_sections WHERE assessment_id = ?', [assessmentId]);
        const sectionIds = sections.map(s => s.id);
        ...
        // Find all questions
        const [questions] = await connection.query(`SELECT id, assessment_section_id, question_type_id, title, question_text, explanation, point, question_image, question_order 
       FROM questions 
       WHERE assessment_section_id IN (${sectionIds.map(() => '?').join(',')}) AND status = 'ACTIVE'
       ORDER BY question_order ASC, id ASC`, sectionIds);
        const questionIds = questions.map(q => q.id);
        // Find all options including correct status
        const [allOptions] = await connection.query(`SELECT id, question_id, option_label, option_text, is_correct, score, option_image, option_order 
       FROM question_options 
       WHERE question_id IN (${questionIds.map(() => '?').join(',')})
       ORDER BY option_order ASC, id ASC`, questionIds);
        ...
        const evaluations = questions.map((q, qIdx) => {
            ...
            const qOptions = allOptions.filter((opt) => opt.question_id === q.id);
            const correctOpt = qOptions.find((opt) => opt.is_correct === 1);
            let isAnswerCorrect = false;
            let earnedPoint = 0;
            let studentAnswerText = '-';
            let correctAnswerText = correctOpt ? `${correctOpt.option_label ? correctOpt.option_label + '. ' : ''}${correctOpt.option_text}` : '-';
            const isBlankAnswer = !userAnsObj || (userAnsObj.selectedOptionId === undefined &&
            ...
            if (isBlankAnswer) {
                ...
                const submittedIds = Array.isArray(userAnsObj.selectedOptionIds)
                    ...
                    : Array.isArray(userAnsObj.selectedOptionId)
                        ...
                        : userAnsObj.selectedOptionId !== undefined
                            ...
                            : [];
                const correctOptIds = qOptions.filter((opt) => opt.is_correct === 1).map((opt) => opt.id);
                const chosenOpts = qOptions.filter((opt) => submittedIds.includes(opt.id));
                studentAnswerText = chosenOpts.length > 0
                    ? chosenOpts.map((opt) => `${opt.option_label ? opt.option_label + '. ' : ''}${opt.option_text}`).join(', ')
                    : '(Tidak dijawab)';
                const correctOpts = qOptions.filter((opt) => opt.is_correct === 1);
                correctAnswerText = correctOpts.map((opt) => `${opt.option_label ? opt.option_label + '. ' : ''}${opt.option_text}`).join(', ');
                ...
                if (Array.isArray(rawMatching)) {
                    submittedPairs = rawMatching.map((item) => {
                        let mText = '';
                        if (item.matchText !== undefined)
                            mText = String(item.matchText);
                        else if (item.selectedMatch !== undefined)
                            mText = String(item.selectedMatch);
                        else if (item.match !== undefined)
                            mText = String(item.match);
                        else if (item.right !== undefined)
                            mText = String(item.right);
                        else if (item.optionText !== undefined)
                            mText = String(item.optionText);
                        else if (item.option_text !== undefined)
                            mText = String(item.option_text);
                        else if (item.answer !== undefined)
                            mText = String(item.answer);
                        else if (item.target !== undefined)
                            mText = String(item.target);
                        if (!mText && (item.matchOptionId || item.rightOptionId || item.targetOptionId)) {
                            ...
                            const foundOpt = qOptions.find((o) => o.id === targetId);
                            if (foundOpt)
                                mText = String(foundOpt.option_text);
                        }
                        return {
                    ...
                    submittedPairs = Object.entries(rawMatching).map(([key, val]) => {
                        ...
                        if (!isNaN(Number(mText))) {
                            const foundOpt = qOptions.find((o) => o.id === Number(mText));
                            if (foundOpt)
                                mText = String(foundOpt.option_text).trim();
                        }
                        return {
                ...
                const studentPairStrs = [];
                const correctPairStrs = [];
                for (const opt of qOptions) {
                    correctPairStrs.push(`${opt.option_label} ➔ ${opt.option_text}`);
                    const studentPair = submittedPairs.find(sp => (sp.optionId !== undefined && sp.optionId === opt.id) ||
                ...
                const chosenOpt = qOptions.find((opt) => opt.id === Number(userAnsObj?.selectedOptionId));
                if (chosenOpt) {
            ...
            if (q.question_type_id === 5) {
                ...
                for (const opt of qOptions) {
                    const sp = submittedPairs.find(p => (p.optionId !== undefined && p.optionId === opt.id) ||
        ...
        const durationSeconds = Number(req.body?.durationSeconds || req.body?.duration_seconds) > 0
            ...
            : 300;
        const [attemptResult] = await connection.query(`INSERT INTO assessment_attempts 
       ...
       VALUES (?, ?, DATE_SUB(NOW(), INTERVAL ? SECOND), NOW(), ?, ?, ?, ?, ?, ?, ?, 'FINISHED')`, [assessmentId, req.user.id, durationSeconds, durationSeconds, score, totalCorrect, totalWrong, totalUnanswered, percentage, passed]);
        const attemptId = attemptResult.insertId;
        // 5. Update assessment progress for this user
        const [existingProgress] = await connection.query('SELECT id, highest_score, total_attempt, passed FROM assessment_progress WHERE user_id = ? AND assessment_id = ?', [req.user.id, assessmentId]);
        if (existingProgress.length > 0) {
            ...
            await connection.query(`UPDATE assessment_progress 
         SET last_attempt = ?, highest_score = ?, total_attempt = ?, passed = ? 
         WHERE user_id = ? AND assessment_id = ?`, [attemptId, highestScore, totalAttempts, alreadyPassed, req.user.id, assessmentId]);
        }
        else {
            await connection.query(`INSERT INTO assessment_progress 
         (assessment_id, user_id, last_attempt, highest_score, total_attempt, passed) 
         VALUES (?, ?, ?, ?, 1, ?)`, [assessmentId, req.user.id, attemptId, percentage, passed]);
        }
        let awardXp = false;
        if (passed === 1) {
        ...
        if (awardXp) {
            await (0, xp_1.addXpTransaction)(connection, req.user.id, 'QUIZ', 30, assessmentId, `Passed Quiz #${assessmentId}`);
            // Update statistics
            await connection.query(`INSERT INTO user_statistics (user_id, total_quiz) 
         VALUES (?, 1) 
         ON DUPLICATE KEY UPDATE 
         total_quiz = total_quiz + 1`, [req.user.id]);
        }
        // Automatically complete the parent lesson when student submits an attempt
        if (quiz.lesson_id) {
            await (0, progress_1.updateProgressHelper)(connection, req.user.id, quiz.lesson_id, true, 100.00);
        }
        await connection.commit();
        // Auto-capture vocabulary into student's personal vocabulary room
        let vocabResults = [];
        try {
            vocabResults = await (0, vocabularyService_1.extractVocabFromQuiz)(req.user.id, assessmentId, quiz.title || `Quiz #${assessmentId}`, questions, answers);
        }
        catch (vErr) {
...
const getAttemptHistory = async (req, res) => {
    ...
    `;
        ...
        const [attempts] = await db_1.default.query(query, params);
        res.json(attempts);
    }
    catch (error) {
        res.status(500).json({ error: error.message || 'Internal server error' });
...
const getAttemptDetails = async (req, res) => {
    ...
    try {
        ...
        const [attempts] = await db_1.default.query(`SELECT aa.*, a.title as quiz_title, a.show_answer 
       FROM assessment_attempts aa
       JOIN assessments a ON aa.assessment_id = a.id
       WHERE aa.id = ? AND aa.user_id = ?`, [attemptId, req.user.id]);
        ...
        const [sections] = await db_1.default.query('SELECT id, title FROM assessment_sections WHERE assessment_id = ?', [attempt.assessment_id]);
        let questions = [];
        if (sections.length > 0) {
            const sectionIds = sections.map(s => s.id);
            const [questionRows] = await db_1.default.query(`SELECT id, title, question_text, explanation, point 
         FROM questions 
         WHERE assessment_section_id IN (${sectionIds.map(() => '?').join(',')}) AND status = 'ACTIVE'`, sectionIds);
            questions = questionRows;
            if (questions.length > 0) {
                const questionIds = questions.map(q => q.id);
                // Fetch all options
                ...
                const [optionRows] = await db_1.default.query(`SELECT ${fields} FROM question_options WHERE question_id IN (${questionIds.map(() => '?').join(',')})`, questionIds);
                for (const q of questions) {
...
const getAssessmentsByType = async (req, res) => {
    ...
    try {
        const [types] = await db_1.default.query('SELECT id FROM assessment_types WHERE code = ?', [typeCode]);
        if (types.length === 0) {
            res.status(404).json({ error: 'Assessment type not found' });
            return;
        }
        const typeId = types[0].id;
        const [rows] = await db_1.default.query('SELECT id, title, description, instruction, passing_score, duration_minutes, total_question, total_score, status FROM assessments WHERE assessment_type_id = ? AND status = "PUBLISHED"', [typeId]);
        res.json(rows);
    }
    catch (error) {
        res.status(500).json({ error: error.message || 'Internal server error' });
...
exports.getAssessmentsByType = getAssessmentsByType;
/**
 ...
 */
const checkSingleQuestion = async (req, res) => {
    ...
    try {
        ...
        const [qRows] = await db_1.default.query(`SELECT id, assessment_section_id, question_type_id, title, question_text, explanation, point, question_image 
       FROM questions 
       WHERE id = ?`, [questionId]);
        if (qRows.length === 0) {
        ...
        const [optionRows] = await db_1.default.query(`SELECT id, question_id, option_label, option_text, is_correct, score, option_order, option_image 
       FROM question_options 
       WHERE question_id = ? 
       ORDER BY option_order ASC`, [questionId]);
        let isAnswerCorrect = false;
        let earnedPoint = 0;
        let studentAnswerText = '';
        ...
        if (q.question_type_id === 1 || q.question_type_id === 3) {
            ...
            if (Array.isArray(rawMatching)) {
                submittedPairs = rawMatching.map((item) => {
                    ...
                    if (!mText && (item.matchOptionId || item.rightOptionId || item.targetOptionId)) {
                        const targetId = Number(item.matchOptionId || item.rightOptionId || item.targetOptionId);
                        const foundOpt = optionRows.find((o) => o.id === targetId);
                        if (foundOpt)
                            mText = String(foundOpt.option_text);
                    }
                    return {
                ...
                submittedPairs = Object.entries(rawMatching).map(([key, val]) => {
                    let mText = String(val).trim();
                    if (!isNaN(Number(mText))) {
                        const foundOpt = optionRows.find((o) => o.id === Number(mText));
                        if (foundOpt)
                            mText = String(foundOpt.option_text).trim();
                    }
                    return {
        ...
        let vocabCaptured = null;
        try {
            if (q.question_type_id === 5) {
                for (const pr of pairResults) {
                    if (pr.label && pr.correctMatch) {
                        vocabCaptured = await (0, vocabularyService_1.recordVocabularyItem)({
                            userId: req.user.id,
...
exports.checkSingleQuestion = checkSingleQuestion;
