...
exports.completeOnboarding = exports.socialLogin = exports.updateProfile = exports.forgotPassword = exports.getProfile = exports.login = exports.resendCode = exports.verifyCode = exports.register = void 0;
...
const register = async (req, res) => {
    ...
    try {
        ...
        const [existing] = await connection.query('SELECT id, status FROM users WHERE email = ? OR username = ?', [email, username]);
        ...
        if (existing.length > 0) {
            ...
            await connection.query(`UPDATE users SET full_name = ?, password = ?, verification_code = ?, verification_expires = DATE_ADD(NOW(), INTERVAL 15 MINUTE) WHERE id = ?`, [fullName, hashedPassword, verificationCode, userExist.id]);
        ...
        const [userResult] = await connection.query('INSERT INTO users (role_id, full_name, email, username, password, status, verification_code, verification_expires) VALUES (?, ?, ?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL 15 MINUTE))', [4, fullName, email, username, hashedPassword, 'PENDING', verificationCode]);
...
const verifyCode = async (req, res) => {
    ...
    try {
        const [users] = await db_1.default.query('SELECT id, role_id, full_name, email, username, status, verification_code, verification_expires FROM users WHERE email = ?', [email]);
        if (users.length === 0) {
        ...
        if (user.status === 'ACTIVE') {
            const token = jsonwebtoken_1.default.sign({ id: user.id, roleId: user.role_id, email: user.email, username: user.username }, process.env.JWT_SECRET || 'supersecret_pronunciation_ai_key', { expiresIn: '24h' });
            res.json({
        ...
        // Activate user
        await db_1.default.query('UPDATE users SET status = "ACTIVE", verification_code = NULL, verification_expires = NULL WHERE id = ?', [user.id]);
        // Initialize user_statistics
        await db_1.default.query('INSERT INTO user_statistics (user_id, xp, level) VALUES (?, 0, 1) ON DUPLICATE KEY UPDATE user_id = user_id', [user.id]);
        const token = jsonwebtoken_1.default.sign({ id: user.id, roleId: user.role_id, email: user.email, username: user.username }, process.env.JWT_SECRET || 'supersecret_pronunciation_ai_key', { expiresIn: '24h' });
        res.json({
...
const resendCode = async (req, res) => {
    ...
    try {
        const [users] = await db_1.default.query('SELECT id, full_name, email, status FROM users WHERE email = ?', [email]);
        if (users.length === 0) {
            res.status(404).json({ error: 'Akun tidak ditemukan' });
            return;
        }
        ...
        await db_1.default.query('UPDATE users SET verification_code = ?, verification_expires = DATE_ADD(NOW(), INTERVAL 15 MINUTE) WHERE id = ?', [newCode, user.id]);
...
const login = async (req, res) => {
    ...
    try {
        const [users] = await db_1.default.query('SELECT id, role_id, full_name, email, username, password, status FROM users WHERE email = ?', [email]);
        if (users.length === 0) {
        ...
        const [statsRows] = await db_1.default.query('SELECT xp, level FROM user_statistics WHERE user_id = ?', [user.id]);
        let xp = Number(statsRows[0]?.xp || 0);
        // If XP is 0 in stats, check if there are current-season transactions
        if (xp === 0 && activeSeason) {
            const [txRows] = await db_1.default.query('SELECT COALESCE(SUM(xp), 0) as total_xp FROM xp_transactions WHERE user_id = ? AND season_id = ?', [user.id, activeSeason.id]);
            const totalTxXp = Number(txRows[0]?.total_xp || 0);
            if (totalTxXp > 0) {
                xp = totalTxXp;
            }
        }
        const progression = await (0, levelService_1.getLevelByXp)(xp);
        const level = progression.level;
        await db_1.default.query('INSERT INTO user_statistics (user_id, xp, level) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE xp = ?, level = ?', [user.id, xp, level, xp, level]);
        res.json({
...
const getProfile = async (req, res) => {
    ...
    try {
        const [profileData] = await db_1.default.query(`SELECT u.id, u.full_name, u.email, u.username, u.avatar, u.phone, 
       ...
       WHERE u.id = ?`, [req.user.id]);
        ...
        const [txRows] = await db_1.default.query('SELECT COALESCE(SUM(xp), 0) as total_tx_xp FROM xp_transactions WHERE user_id = ? AND (season_id = ? OR season_id IS NULL)', [req.user.id, activeSeason?.id || 0]);
        const totalTxXp = Number(txRows[0]?.total_tx_xp || 0);
        const [statsRows] = await db_1.default.query('SELECT xp, level FROM user_statistics WHERE user_id = ?', [req.user.id]);
        ...
        await db_1.default.query(`INSERT INTO user_statistics (user_id, xp, level) VALUES (?, ?, ?)
       ON DUPLICATE KEY UPDATE xp = ?, level = ?`, [req.user.id, xp, level, xp, level]);
        // Fetch real enrollments with progress
        const [enrollmentRows] = await db_1.default.query(`SELECT e.id, e.course_id, c.title as course_title, c.slug as course_slug, COALESCE(MAX(cp.overall_progress), 0) as progress_percent
       FROM enrollments e
       ...
       GROUP BY e.id, e.course_id, c.title, c.slug`, [req.user.id]);
        res.json({
            ...profileData[0],
            xp,
            level,
...
const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const [users] = await db_1.default.query('SELECT id, full_name, email, username FROM users WHERE email = ?', [email]);
        if (users.length === 0) {
        ...
        const newPasswordHash = await bcryptjs_1.default.hash(newPasswordPlain, 10);
        // 2. Reset user's password in database to barupassword123
        await db_1.default.query('UPDATE users SET password = ? WHERE id = ?', [newPasswordHash, targetUser.id]);
...
const updateProfile = async (req, res) => {
    ...
    const { fullName, username, email, phone, avatar, oldPassword, newPassword, gender, birthDate, country, province, city, address, postalCode, bio, profession, education, organization } = req.body;
    try {
        // 1. Fetch existing user
        const [users] = await db_1.default.query('SELECT * FROM users WHERE id = ?', [userId]);
        if (users.length === 0) {
        ...
        if (newPassword) {
            ...
            const isMatch = await bcryptjs_1.default.compare(oldPassword, currentUser.password);
            if (!isMatch) {
            ...
            passwordHash = await bcryptjs_1.default.hash(newPassword, 10);
        }
        // 3. Check username / email uniqueness if changed
        if (username && username !== currentUser.username) {
            const [existingUser] = await db_1.default.query('SELECT id FROM users WHERE username = ? AND id != ?', [username, userId]);
        ...
        if (email && email !== currentUser.email) {
            const [existingEmail] = await db_1.default.query('SELECT id FROM users WHERE email = ? AND id != ?', [email, userId]);
       ...
       WHERE id = ?`, [
        ...
        await db_1.default.query(`INSERT INTO user_profiles (user_id, gender, birth_date, country, province, city, address, postal_code, bio, profession, education, organization)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ...
       organization = VALUES(organization)`, [
        ...
        const [updatedData] = await db_1.default.query(`SELECT u.id, u.role_id, u.full_name, u.email, u.username, u.avatar, u.phone,
              p.gender, p.birth_date, p.country, p.province, p.city, p.address, p.postal_code, p.bio, p.profession, p.education, p.organization
       FROM users u
       LEFT JOIN user_profiles p ON u.id = p.user_id
       WHERE u.id = ?`, [userId]);
        const [statsRows] = await db_1.default.query('SELECT xp, level FROM user_statistics WHERE user_id = ?', [userId]);
        const currentXp = Number(statsRows[0]?.xp || 0);
        const progression = await (0, levelService_1.getLevelByXp)(currentXp);
        res.json({
...
const socialLogin = async (req, res) => {
    ...
    try {
        ...
        const [existing] = await db_1.default.query(`SELECT u.id, u.role_id, u.full_name, u.email, u.username, u.avatar, r.name as role_name 
       FROM users u 
       ...
       WHERE u.email = ? LIMIT 1`, [email]);
        ...
        if (existing.length > 0) {
            ...
            const dummyPass = await bcryptjs_1.default.hash(`social_${provider}_${uid}_${Date.now()}`, 10);
            const [insertResult] = await db_1.default.query(`INSERT INTO users (role_id, full_name, email, username, password, avatar, status) 
         VALUES (?, ?, ?, ?, ?, ?, 'active')`, [4, fullName || 'Siswa Social', email, uniqueUsername, dummyPass, photoURL || null]);
            const newUserId = insertResult.insertId;
            // Seed statistics for student
            await db_1.default.query('INSERT IGNORE INTO user_statistics (user_id, xp, level) VALUES (?, 0, 1)', [newUserId]);
            userObj = {
                id: newUserId,
                role_id: 4,
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ id: userObj.id, role_id: userObj.role_id, email: userObj.email }, process.env.JWT_SECRET || 'supersecret_pronunciation_ai_key', { expiresIn: '7d' });
        // Fetch statistics
        const [stats] = await db_1.default.query('SELECT xp, level FROM user_statistics WHERE user_id = ?', [userObj.id]);
        res.json({
...
const completeOnboarding = async (req, res) => {
    ...
    try {
        ...
        if (username) {
            const [userCheck] = await db_1.default.query('SELECT id FROM users WHERE username = ? AND id != ? LIMIT 1', [username, userId]);
        ...
        if (updateFields.length > 0) {
            ...
            await db_1.default.query(`UPDATE users SET ${updateFields.join(', ')} WHERE id = ?`, queryParams);
        }
        // Fetch updated user
        const [updatedRows] = await db_1.default.query(`SELECT u.id, u.role_id, u.full_name, u.email, u.username, u.avatar, u.phone, r.name as role_name 
       FROM users u 
       JOIN roles r ON u.role_id = r.id 
       WHERE u.id = ?`, [userId]);
        const [stats] = await db_1.default.query('SELECT xp, level FROM user_statistics WHERE user_id = ?', [userId]);
