...
exports.getStudentOrders = exports.updatePaymentSettings = exports.getPaymentSettings = exports.ensurePaymentSettingsTable = exports.handleFlipWebhook = exports.createFlipPaymentLink = exports.submitManualPaymentProof = exports.ensureManualPaymentProofsTable = exports.handleManualScalevSync = exports.syncScalevOrdersFromApi = exports.processScalevWebhook = exports.determineScalevAccessDays = exports.processPaymentCallback = exports.createOrder = void 0;
...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        const [courses] = await connection.query('SELECT id, price, discount_price, status FROM courses WHERE id = ?', [courseId]);
        if (courses.length === 0) {
        ...
        const [existingEnrollment] = await connection.query("SELECT id FROM enrollments WHERE user_id = ? AND course_id = ? AND status = 'ACTIVE'", [req.user.id, courseId]);
        ...
        const [orderResult] = await connection.query('INSERT INTO orders (order_number, user_id, subtotal, discount, tax, grand_total, payment_status, order_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [orderNumber, req.user.id, subtotal, discount, tax, grandTotal, paymentStatus, orderStatus]);
        const orderId = orderResult.insertId;
        // 5. Create Order Item
        await connection.query('INSERT INTO order_items (order_id, course_id, price, discount, total) VALUES (?, ?, ?, ?, ?)', [orderId, course.id, price, discount, grandTotal]);
        if (isFreeCourse) {
            // 6. Auto Enroll Free Course
            const extTxId = `FREE-${Date.now()}-${Math.floor(1000 + Math.random() * 9000)}`;
            const [paymentResult] = await connection.query('INSERT INTO payments (order_id, payment_method_id, external_transaction_id, amount, paid_at, status) VALUES (?, ?, ?, ?, NOW(), ?)', [orderId, 1, extTxId, 0, 'SUCCESS']);
            const paymentId = paymentResult.insertId;
            await connection.query('INSERT INTO payment_transactions (payment_id, gateway_name, gateway_reference, request_payload, response_payload) VALUES (?, ?, ?, ?, ?)', [paymentId, 'FREE_ENROLLMENT', extTxId, JSON.stringify(req.body), JSON.stringify({ status: 'SUCCESS', message: 'Free Course Enrollment Approved' })]);
            const accessDays = course.access_days || 365;
            const [enrollResult] = await connection.query('INSERT INTO enrollments (user_id, course_id, source_id, order_id, enrolled_at, expired_at, access_days, status) VALUES (?, ?, 1, ?, NOW(), DATE_ADD(NOW(), INTERVAL ? DAY), ?, ?)', [req.user.id, course.id, orderId, accessDays, accessDays, 'ACTIVE']);
            const enrollmentId = enrollResult.insertId;
            await connection.query("INSERT INTO enrollment_histories (enrollment_id, action, description, created_by) VALUES (?, 'CREATE', 'Auto-enrolled in free course', ?)", [enrollmentId, req.user.id]);
            const [modulesCount] = await connection.query(`SELECT COUNT(*) as count 
         FROM modules m 
         JOIN course_versions cv ON m.course_version_id = cv.id 
         WHERE cv.course_id = ? AND cv.is_current = 1 AND m.status = 'PUBLISHED'`, [course.id]);
            const totalModules = modulesCount.length > 0 ? modulesCount[0].count : 0;
            await connection.query('INSERT INTO course_progress (user_id, course_id, enrollment_id, completed_module, total_module, overall_progress, average_score) VALUES (?, ?, ?, 0, ?, 0.00, 0.00)', [req.user.id, course.id, enrollmentId, totalModules]);
        }
...
const processPaymentCallback = async (req, res) => {
    ...
    try {
        ...
        const [orders] = await connection.query('SELECT id, user_id, grand_total, payment_status FROM orders WHERE id = ?', [orderId]);
        if (orders.length === 0) {
        ...
        if (status === 'SUCCESS') {
            ...
            await connection.query("UPDATE orders SET payment_status = 'PAID', order_status = 'SUCCESS' WHERE id = ?", [orderId]);
            ...
            const [paymentResult] = await connection.query('INSERT INTO payments (order_id, payment_method_id, external_transaction_id, amount, paid_at, status) VALUES (?, ?, ?, ?, NOW(), ?)', [orderId, 1, extTxId, order.grand_total, 'SUCCESS']);
            const paymentId = paymentResult.insertId;
            // 4. Create payment transaction record
            await connection.query('INSERT INTO payment_transactions (payment_id, gateway_name, gateway_reference, request_payload, response_payload) VALUES (?, ?, ?, ?, ?)', [paymentId, 'MIDTRANS', extTxId, JSON.stringify(req.body), JSON.stringify({ status: 'SUCCESS', message: 'Approved' })]);
            // 5. Auto Enroll User
            // Fetch course_id from order items
            const [orderItems] = await connection.query('SELECT course_id FROM order_items WHERE order_id = ?', [orderId]);
            if (orderItems.length > 0) {
                ...
                const [enrollResult] = await connection.query('INSERT INTO enrollments (user_id, course_id, source_id, order_id, enrolled_at, expired_at, access_days, status) VALUES (?, ?, 1, ?, NOW(), DATE_ADD(NOW(), INTERVAL ? DAY), ?, ?)', [order.user_id, courseId, orderId, accessDays, accessDays, 'ACTIVE']);
                const enrollmentId = enrollResult.insertId;
                // Insert enrollment history
                await connection.query("INSERT INTO enrollment_histories (enrollment_id, action, description, created_by) VALUES (?, 'CREATE', 'Auto-enrolled after successful purchase', ?)", [enrollmentId, order.user_id]);
                // Fetch count of modules in the current course version
                const [modulesCount] = await connection.query(`SELECT COUNT(*) as count 
           FROM modules m 
           JOIN course_versions cv ON m.course_version_id = cv.id 
           WHERE cv.course_id = ? AND cv.is_current = 1 AND m.status = 'PUBLISHED'`, [courseId]);
                const totalModules = modulesCount.length > 0 ? modulesCount[0].count : 0;
                // Initialize course progress record
                await connection.query('INSERT INTO course_progress (user_id, course_id, enrollment_id, completed_module, total_module, overall_progress, average_score) VALUES (?, ?, ?, 0, ?, 0.00, 0.00)', [order.user_id, courseId, enrollmentId, totalModules]);
            }
            // Create admin notification for new transaction
            try {
                const [userRows] = await connection.query('SELECT full_name FROM users WHERE id = ?', [order.user_id]);
                const buyerName = userRows[0]?.full_name || 'A student';
                const [adminRows] = await connection.query('SELECT id FROM users WHERE role_id IN (1, 2)');
                for (const adminUser of adminRows) {
                    await connection.query(`INSERT INTO user_notifications (user_id, title, message) 
             VALUES (?, ?, ?)`, [
                        adminUser.id,
                        'New Platform Sale',
            ...
            // Payment failed
            await connection.query("UPDATE orders SET payment_status = 'FAILED', order_status = 'FAILED' WHERE id = ?", [orderId]);
        }
        await connection.commit();
        res.json({
 ...
 */
const determineScalevAccessDays = async (itemText, fallbackDays = 365) => {
    const text = String(itemText || '').toLowerCase();
    try {
        const [dbPackages] = await db_1.default.query('SELECT * FROM scalev_packages WHERE status = "ACTIVE" ORDER BY id ASC');
        for (const pkg of dbPackages) {
            const keywords = (pkg.keyword || '').split(',').map((k) => k.trim().toLowerCase()).filter(Boolean);
...
exports.determineScalevAccessDays = determineScalevAccessDays;
/**
 ...
 */
const processScalevWebhook = async (req, res) => {
    const body = req.body || {};
    ...
    const externalOrderId = String(orderData.order_id ||
        orderData.id ||
        ...
        body.unique_id ||
        `SLV-${Date.now()}`);
    const customerEmail = String((typeof orderData.customer === 'object' && orderData.customer?.email) ||
        ...
        body.customer_email ||
        '').trim().toLowerCase();
    const customerName = String((typeof orderData.customer === 'object' && orderData.customer?.name) ||
        ...
        body.customer_name ||
        'Scalev Student').trim();
    const customerPhone = String((typeof orderData.customer === 'object' && orderData.customer?.phone) ||
        ...
        body.customer_phone ||
        '').trim();
    const paymentStatus = String(orderData.payment_status ||
        orderData.status ||
        ...
        body.order_status ||
        'PAID').toUpperCase();
    const grandTotal = Number(orderData.net_revenue ||
    ...
    const isPaid = rawEvent.includes('payment received') ||
    ...
    const items = orderData.final_variants ||
        orderData.variants ||
        orderData.items ||
    ...
    let itemText = Array.isArray(items)
        ? items.map((i) => `${i.product_name || i.variant_name || i.name || i.title || i.sku || ''}`).join(' ')
        : '';
    if (!itemText) {
    ...
    try {
        ...
        const [existingScalev] = await connection.query('SELECT id FROM scalev_orders WHERE external_order_id = ?', [externalOrderId]);
        ...
        let userId;
        const [existingUser] = await connection.query('SELECT id FROM users WHERE email = ?', [customerEmail]);
        if (existingUser.length > 0) {
            ...
            const [userResult] = await connection.query('INSERT INTO users (role_id, full_name, email, username, password, phone, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [4, customerName, customerEmail, defaultUsername, hashedPassword, customerPhone, 'ACTIVE']);
        ...
        if (itemSku) {
            const [foundCourses] = await connection.query('SELECT id FROM courses WHERE code = ? OR slug = ? LIMIT 1', [itemSku, itemSku]);
        ...
        if (courseId === 1 && itemTitle) {
            const [matchedByTitle] = await connection.query('SELECT id FROM courses WHERE ? LIKE CONCAT("%", title, "%") OR title LIKE CONCAT("%", ?, "%") OR status = "PUBLISHED" ORDER BY id ASC LIMIT 1', [itemTitle, itemTitle]);
        ...
        const [orderResult] = await connection.query('INSERT INTO orders (order_number, user_id, subtotal, discount, tax, grand_total, payment_status, order_status) VALUES (?, ?, ?, 0, 0, ?, "PAID", "SUCCESS")', [orderNumber, userId, grandTotal, grandTotal]);
        const orderId = orderResult.insertId;
        await connection.query('INSERT INTO order_items (order_id, course_id, price, discount, total) VALUES (?, ?, ?, 0, ?)', [orderId, courseId, grandTotal, grandTotal]);
        // 5. Insert Scalev Order Tracking Record
        await connection.query('INSERT INTO scalev_orders (external_order_id, order_id, api_key_used, sync_status, synced_at) VALUES (?, ?, "scalev_webhook_live", "SYNCED", NOW())', [externalOrderId, orderId]);
        // 6. Insert Payment Record (dynamically check or create SCALEV payment method)
        const [pmRows] = await connection.query('SELECT id FROM payment_methods WHERE code = "SCALEV" OR provider = "SCALEV" LIMIT 1');
        let pmId = 1;
        if (pmRows.length > 0) {
            pmId = pmRows[0].id;
        }
        else {
            const [newPm] = await connection.query('INSERT INTO payment_methods (code, name, provider, status) VALUES ("SCALEV", "Scalev Gateway", "SCALEV", 1)');
            pmId = newPm.insertId;
        }
        const [paymentResult] = await connection.query('INSERT INTO payments (order_id, payment_method_id, external_transaction_id, amount, paid_at, status) VALUES (?, ?, ?, ?, NOW(), "SUCCESS")', [orderId, pmId, externalOrderId, grandTotal]);
        const paymentId = paymentResult.insertId;
        await connection.query('INSERT INTO payment_transactions (payment_id, gateway_name, gateway_reference, request_payload, response_payload) VALUES (?, "SCALEV", ?, ?, ?)', [paymentId, externalOrderId, JSON.stringify(req.body), JSON.stringify({ status: 'SUCCESS', message: 'Scalev Webhook Processed' })]);
        // 7. Auto-Enroll Student if not already enrolled
        const [existingEnrollment] = await connection.query('SELECT id FROM enrollments WHERE user_id = ? AND course_id = ? AND status = "ACTIVE"', [userId, courseId]);
        if (existingEnrollment.length === 0) {
            const [courses] = await connection.query('SELECT access_days FROM courses WHERE id = ?', [courseId]);
            const defaultCourseDays = courses[0]?.access_days || 365;
            const { accessDays, packageName } = await (0, exports.determineScalevAccessDays)(itemText, defaultCourseDays);
            const [enrollResult] = await connection.query('INSERT INTO enrollments (user_id, course_id, source_id, order_id, enrolled_at, expired_at, access_days, status) VALUES (?, ?, 2, ?, NOW(), DATE_ADD(NOW(), INTERVAL ? DAY), ?, "ACTIVE")', [userId, courseId, orderId, accessDays, accessDays]);
            const enrollmentId = enrollResult.insertId;
            await connection.query('INSERT INTO enrollment_histories (enrollment_id, action, description, created_by) VALUES (?, "CREATE", ?, ?)', [enrollmentId, `Auto-enrolled via Scalev Webhook [${packageName} - Akses ${accessDays} Hari]`, userId]);
            const [modulesCount] = await connection.query('SELECT COUNT(*) as count FROM modules m JOIN course_versions cv ON m.course_version_id = cv.id WHERE cv.course_id = ? AND cv.is_current = 1 AND m.status = "PUBLISHED"', [courseId]);
            const totalModules = modulesCount[0]?.count || 0;
            await connection.query('INSERT INTO course_progress (user_id, course_id, enrollment_id, completed_module, total_module, overall_progress, average_score) VALUES (?, ?, ?, 0, ?, 0.00, 0.00)', [userId, courseId, enrollmentId, totalModules]);
        }
        // 7.5 Send Automated Welcome Email to Buyer with Login Credentials & Package Access Info
        const [enrollmentDetails] = await connection.query('SELECT expired_at FROM enrollments WHERE user_id = ? AND course_id = ? ORDER BY id DESC LIMIT 1', [userId, courseId]);
        const { accessDays: matchedDays, packageName: matchedPkgName } = await (0, exports.determineScalevAccessDays)(itemText);
        (0, emailService_1.sendScalevWelcomeEmail)({
        ...
        const [adminRows] = await connection.query('SELECT id FROM users WHERE role_id IN (1, 2)');
        for (const adminUser of adminRows) {
            await connection.query('INSERT INTO user_notifications (user_id, title, message) VALUES (?, ?, ?)', [adminUser.id, 'New Scalev Transaction', `Scalev Sale: Rp ${grandTotal.toLocaleString('id-ID')} for ${customerName} (${customerEmail}). Order: ${externalOrderId}`]);
        }
        await connection.commit();
        res.json({
            success: true,
...
exports.processScalevWebhook = processScalevWebhook;
/**
 ...
 */
const syncScalevOrdersFromApi = async (apiKey) => {
    ...
    try {
        ...
        for (const orderItem of ordersList) {
            ...
            await (0, exports.processScalevWebhook)(mockReq, mockRes);
        }
        return {
            success: true,
...
exports.syncScalevOrdersFromApi = syncScalevOrdersFromApi;
/**
 * Controller endpoint: POST /api/admin/orders/scalev-sync
 */
const handleManualScalevSync = async (req, res) => {
    const { apiKey } = req.body || {};
    const result = await (0, exports.syncScalevOrdersFromApi)(apiKey);
    res.json(result);
};
exports.handleManualScalevSync = handleManualScalevSync;
// ═══════════════════════════════════════════════════════════
// MANUAL PAYMENT PROOFS & FLIP PAYMENT GATEWAY INTEGRATION
// ═══════════════════════════════════════════════════════════
const ensureManualPaymentProofsTable = async () => {
    try {
...
exports.ensureManualPaymentProofsTable = ensureManualPaymentProofsTable;
const submitManualPaymentProof = async (req, res) => {
    ...
    if (!orderId || !proofImage) {
        res.status(400).json({ error: 'Order ID and Proof Image are required' });
        return;
    }
    try {
        await (0, exports.ensureManualPaymentProofsTable)();
        const [orders] = await db_1.default.query('SELECT id, grand_total, payment_status FROM orders WHERE id = ? AND user_id = ?', [orderId, req.user.id]);
        if (orders.length === 0) {
            res.status(404).json({ error: 'Order not found' });
            return;
        }
        ...
        const [result] = await db_1.default.query(`INSERT INTO manual_payment_proofs (order_id, user_id, bank_name, sender_name, amount, proof_image, notes, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'PENDING')`, [orderId, req.user.id, bankName || 'Bank Transfer', senderName || req.user.email, transferAmount, proofImage, notes || null]);
        res.status(201).json({
...
exports.submitManualPaymentProof = submitManualPaymentProof;
const createFlipPaymentLink = async (req, res) => {
    ...
    try {
        const [orders] = await db_1.default.query(`SELECT o.id, o.order_number, o.grand_total, u.email, u.full_name, GROUP_CONCAT(c.title SEPARATOR ', ') as courseTitle
       FROM orders o
       JOIN users u ON o.user_id = u.id
       JOIN order_items oi ON o.id = oi.order_id
       JOIN courses c ON oi.course_id = c.id
       WHERE o.id = ? AND o.user_id = ?
       GROUP BY o.id`, [orderId, req.user.id]);
        if (orders.length === 0) {
            res.status(404).json({ error: 'Order not found' });
            return;
        }
...
exports.createFlipPaymentLink = createFlipPaymentLink;
const handleFlipWebhook = async (req, res) => {
    try {
        ...
        if (status === 'SUCCESSFUL' || status === 'SUCCESS' || status === 'PAID') {
            const [orders] = await db_1.default.query('SELECT id FROM orders WHERE order_number = ? OR id = ?', [externalId, externalId]);
            if (orders.length > 0) {
                const orderId = orders[0].id;
                const mockReq = { body: { orderId, status: 'SUCCESS' } };
                const mockRes = { json: () => { }, status: () => ({ json: () => { } }) };
                await (0, exports.processPaymentCallback)(mockReq, mockRes);
            }
        }
...
exports.handleFlipWebhook = handleFlipWebhook;
// ═══════════════════════════════════════════════════════════
...
const ensurePaymentSettingsTable = async () => {
    ...
    `);
        const [rows] = await db_1.default.query('SELECT id FROM payment_settings LIMIT 1');
        if (rows.length === 0) {
            await db_1.default.query(`
...
exports.ensurePaymentSettingsTable = ensurePaymentSettingsTable;
const getPaymentSettings = async (req, res) => {
    try {
        await (0, exports.ensurePaymentSettingsTable)();
        const [rows] = await db_1.default.query('SELECT * FROM payment_settings LIMIT 1');
        res.json(rows[0] || {
...
const updatePaymentSettings = async (req, res) => {
    const { enable_manual_payment, enable_automatic_payment, bank_bca_number, bank_bca_holder, bank_mandiri_number, bank_mandiri_holder, cs_whatsapp_number } = req.body;
    try {
        await (0, exports.ensurePaymentSettingsTable)();
        await db_1.default.query(`
