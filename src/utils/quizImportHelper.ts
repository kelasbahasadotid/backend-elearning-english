...
export const QUESTION_TYPE_MAP: Record<string, { id: number; name: string }> = {
 ...
 */
export function getQuestionTypeIdFromSheetName(sheetName: string): number | null {
  ...
  if (s.includes('multiple choice') || s.includes('pilihan ganda') || s.includes('mcq') || s.startsWith('1')) return 1;
  if (s.includes('multiple select') || s.includes('multi jawaban') || s.includes('checkbox') || s.startsWith('2')) return 2;
  if (s.includes('true') || s.includes('false') || s.includes('benar') || s.includes('salah') || s.includes('tf') || s.startsWith('3')) return 3;
  if (s.includes('blank') || s.includes('isian') || s.includes('esai') || s.startsWith('4')) return 4;
  if (s.includes('match') || s.includes('jodoh') || s.includes('pasang') || s.startsWith('5')) return 5;
  if (s.includes('order') || s.includes('susun') || s.includes('kata') || s.startsWith('6')) return 6;
  return null;
}
 ...
 */
export function parseQuizRows(rawRows: any[], defaultTypeId?: number, sourceSheet?: string): ParsedQuestion[] {
  ...
  rawRows.forEach((rawRow, idx) => {
    ...
    if (rawType && QUESTION_TYPE_MAP[rawType]) {
      questionTypeId = QUESTION_TYPE_MAP[rawType].id;
      typeName = QUESTION_TYPE_MAP[rawType].name;
    } else if (defaultTypeId) {
      const foundEntry = Object.values(QUESTION_TYPE_MAP).find(t => t.id === defaultTypeId);
      if (foundEntry) {
    ...
    // 3: True & False
      ...
      const isTrueCorrect =
        rawCorrect.toLowerCase().includes('true') ||
    ...
    // 6: Word Ordering (Menyusun Kata)
      ...
      for (let w = 1; w <= 12; w++) {
        ...
        if (val) words.push(String(val).trim());
      }
      ...
      if (words.length === 0 && fullSentence) {
        fullSentence.split(/\s+/).forEach((w: string) => {
 ...
 */
export function parseQuizWorkbook(wb: XLSX.WorkBook): ParsedQuestion[] {
 ...
 */
export function createExcelTemplateBuffer(filterType?: string): Buffer {
 ...
 */
export function createCsvTemplateString(filterType?: string): string {
  const rows = [
    {
