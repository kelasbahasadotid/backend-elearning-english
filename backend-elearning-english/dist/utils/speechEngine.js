...
exports.getWordIPA = getWordIPA;
exports.generatePronunciationGuide = generatePronunciationGuide;
exports.analyzeSpeechAccuracy = analyzeSpeechAccuracy;
exports.transcribeAndAnalyze = transcribeAndAnalyze;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const phonemize_1 = require("phonemize");
...
function getOrInitVoskModel() {
    ...
    const modelDirCandidates = [
        path_1.default.join(process.cwd(), 'model'),
        path_1.default.join(process.cwd(), 'vosk-model'),
        path_1.default.join(__dirname, '../../model')
    ];
    const foundDir = modelDirCandidates.find((d) => fs_1.default.existsSync(d));
...
function getWordIPA(word) {
    ...
    if (!clean)
        return '';
    try {
        const ipa = (0, phonemize_1.toIPA)(clean);
        if (ipa && ipa.trim()) {
 ...
 */
function generatePronunciationGuide(word, ipa) {
 ...
 */
function alignWordSequences(promptWords, spokenWords) {
    ...
    const dp = Array.from({ length: M + 1 }, () => Array(N + 1).fill(0));
    for (let i = 0; i <= M; i++)
        dp[i][0] = i * GAP_PROMPT_PENALTY;
    for (let j = 0; j <= N; j++)
        dp[0][j] = j * GAP_SPOKEN_PENALTY;
    for (let i = 1; i <= M; i++) {
        for (let j = 1; j <= N; j++) {
            ...
            dp[i][j] = Math.max(dp[i - 1][j - 1] + pairScore, dp[i - 1][j] + GAP_PROMPT_PENALTY, dp[i][j - 1] + GAP_SPOKEN_PENALTY);
        }
    }
    let i = M;
    let j = N;
 ...
 */
function analyzeSpeechAccuracy(promptText, transcribedText) {
    ...
    if (transcribedWords.length === 0 || cleanTranscript.includes('---')) {
        const wordDetails = promptWords.map((word) => {
    ...
    const overallScore = Math.max(0, Math.min(100, Math.round(pronunciation * 0.40 +
        fluency * 0.30 +
        grammar * 0.15 +
        vocabulary * 0.15)));
 ...
 */
async function transcribeAndAnalyze(audioPath, promptText, durationSeconds = 15, browserTranscript) {
    ...
    if (model && VoskRecognizer && fs_1.default.existsSync(audioPath)) {
        try {
            ...
            let fileBuffer = fs_1.default.readFileSync(audioPath);
            ...
            if (audioPath.toLowerCase().endsWith('.mp3') ||
                ...
                try {
                    const converted = await (0, audioUtils_1.convertMp3ToWav)(fileBuffer);
