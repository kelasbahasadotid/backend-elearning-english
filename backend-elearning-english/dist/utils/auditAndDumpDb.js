    ...
    try {
      ...
      `);
        ...
        for (const table of tables) {
            const [cntRes] = await connection.query(`SELECT COUNT(*) as count FROM \`${table}\``);
            ...
            const [rowsRes] = await connection.query(`SELECT * FROM \`${table}\``);
            tableData[table] = {
                createSql,
                rowCount,
                rows: rowsRes
...
\n`;
        ...
        const backendSqlPath = path_1.default.join(__dirname, '../../database.sql');
        const rootSqlPath = path_1.default.join(__dirname, '../../../database.sql');
        fs_1.default.writeFileSync(backendSqlPath, fullSql, 'utf-8');
        fs_1.default.writeFileSync(rootSqlPath, fullSql, 'utf-8');
