...
exports.verifyCertificate = exports.claimCertificate = void 0;
...
const claimCertificate = async (req, res) => {
    ...
    try {
        ...
        // 1. Fetch enrollment
        const [enrollments] = await connection.query("SELECT id FROM enrollments WHERE user_id = ? AND course_id = ? AND status = 'ACTIVE' LIMIT 1", [req.user.id, courseId]);
        ...
        const [progress] = await connection.query('SELECT overall_progress, certificate_ready FROM course_progress WHERE user_id = ? AND course_id = ? LIMIT 1', [req.user.id, courseId]);
        ...
        const [existingCert] = await connection.query(`SELECT c.id, c.issued_at, c.verification_code, cn.certificate_number,
       ...
       WHERE c.user_id = ? AND c.course_id = ? AND c.status = 'ACTIVE'`, [req.user.id, courseId]);
        // Fetch quiz and speaking grades for the certificate transcript
        const [quizAttempts] = await connection.query(`SELECT a.title, MAX(ap.highest_score) as bestScore, MAX(ap.passed) as passed
       FROM assessments a
       LEFT JOIN assessment_progress ap ON ap.assessment_id = a.id AND ap.user_id = ?
       WHERE a.course_id = ?
       GROUP BY a.id, a.title`, [req.user.id, courseId]);
        const [speakingAttempts] = await connection.query(`SELECT st.title, MAX(sa.overall_score) as bestScore
       ...
       WHERE a.course_id = ?
       GROUP BY st.id, st.title`, [req.user.id, courseId]);
        ...
        const [latestNumber] = await connection.query('SELECT MAX(running_number) as max_num FROM certificate_numbers WHERE year = ? AND month = ?', [year, month]);
        ...
        const [certNumberResult] = await connection.query('INSERT INTO certificate_numbers (certificate_number, running_number, year, month) VALUES (?, ?, ?, ?)', [certNumStr, nextNum, year, month]);
        ...
        const [templates] = await connection.query("SELECT id, background_image, orientation, paper_size, template_name FROM certificate_templates WHERE course_id = ? AND active = 1 LIMIT 1", [courseId]);
        ...
        if (templates.length > 0) {
            ...
            const [defaultTemplates] = await connection.query("SELECT id, background_image, orientation, paper_size, template_name FROM certificate_templates WHERE active = 1 ORDER BY id ASC LIMIT 1");
        ...
        await connection.query(`INSERT INTO certificates 
       ...
       VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, 'ACTIVE')`, [certNumberId, req.user.id, courseId, enrollmentId, templateId, verificationCode, pdfPath, qrCodePath]);
...
const verifyCertificate = async (req, res) => {
    ...
    try {
        const [certs] = await db_1.default.query(`SELECT c.id, c.user_id, c.issued_at, c.verification_code, cn.certificate_number, 
       ...
       WHERE (c.verification_code = ? OR cn.certificate_number = ? OR c.verification_code LIKE ? OR cn.certificate_number LIKE ?) 
         AND (c.status IS NULL OR c.status = '' OR UPPER(c.status) IN ('ACTIVE', 'ISSUED', 'PUBLISHED', '1')) LIMIT 1`, [code, code, `%${code}%`, `%${code}%`]);
        if (certs.length === 0) {
            res.status(404).json({ error: 'Kode verifikasi sertifikat tidak ditemukan atau tidak valid' });
            return;
        }
        const cert = certs[0];
        const [quizAttempts] = await db_1.default.query(`SELECT a.title, MAX(ap.highest_score) as bestScore, MAX(ap.passed) as passed
       FROM assessments a
       LEFT JOIN assessment_progress ap ON ap.assessment_id = a.id AND ap.user_id = ?
       WHERE a.course_id = ?
       GROUP BY a.id, a.title`, [cert.user_id, cert.courseId]);
        const [speakingAttempts] = await db_1.default.query(`SELECT st.title, MAX(sa.overall_score) as bestScore
       FROM speaking_tests st
       JOIN assessments a ON st.assessment_id = a.id
       LEFT JOIN speaking_attempts sa ON sa.speaking_test_id = st.id AND sa.user_id = ?
       WHERE a.course_id = ?
       GROUP BY st.id, st.title`, [cert.user_id, cert.courseId]);
        res.json({
            valid: true,
