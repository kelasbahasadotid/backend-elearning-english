...
exports.uploadH5PMedia = exports.submitH5PAttempt = exports.attachH5PToLesson = exports.deleteH5P = exports.updateH5P = exports.createH5P = exports.getH5PById = exports.getAllH5P = void 0;
...
exports.getAllH5P = getAllH5P;
// 2. Get Single H5P Material by ID
const getH5PById = async (req, res) => {
    const { id } = req.params;
    try {
        await ensureH5PTables();
        const [rows] = await db_1.default.query(`
...
const syncH5PToLessonContents = async (h5pId, lessonId, title, paramsJson) => {
    ...
    `, [lessonId, `%"h5p_id":${h5pId}}%`, contentTitle]);
        if (existing.length === 0) {
            const [maxOrderRow] = await db_1.default.query('SELECT COALESCE(MAX(content_order), 0) as max_order FROM lesson_contents WHERE lesson_id = ?', [lessonId]);
...
const createH5P = async (req, res) => {
    ...
    try {
        await ensureH5PTables();
        if (lesson_id) {
            const [lRows] = await db_1.default.query('SELECT lesson_type FROM lessons WHERE id = ?', [lesson_id]);
...
exports.createH5P = createH5P;
// 4. Update H5P Interactive Material
const updateH5P = async (req, res) => {
    const { id } = req.params;
    const { title, content_type, course_id, lesson_id, params_json, custom_css, theme_color, status } = req.body;
    try {
        await ensureH5PTables();
        const [existing] = await db_1.default.query('SELECT * FROM h5p_contents WHERE id = ?', [id]);
        if (existing.length === 0) {
            return res.status(404).json({ error: 'Materi H5P tidak ditemukan' });
        }
        ...
        if (finalLessonId) {
            const [lRows] = await db_1.default.query('SELECT lesson_type FROM lessons WHERE id = ?', [finalLessonId]);
            if (lRows.length > 0 && lRows[0].lesson_type?.toUpperCase() !== 'READING') {
                return res.status(400).json({ error: 'Materi H5P hanya dapat ditautkan ke sub lesson bertipe READING' });
            }
        }
...
exports.updateH5P = updateH5P;
// 5. Delete H5P Interactive Material
const deleteH5P = async (req, res) => {
    const { id } = req.params;
    try {
        await ensureH5PTables();
        const [rows] = await db_1.default.query('SELECT lesson_id, title FROM h5p_contents WHERE id = ?', [id]);
        const lessonId = rows[0]?.lesson_id ? Number(rows[0].lesson_id) : null;
        if (lessonId) {
            await db_1.default.query(`DELETE FROM lesson_contents
         WHERE lesson_id = ? AND content_type = 'H5P'
           AND (attachments LIKE ? OR title = ?)`, [lessonId, `%"h5p_id":${Number(id)}}%`, `H5P: ${rows[0].title}`]);
        }
        await db_1.default.query('DELETE FROM h5p_student_attempts WHERE h5p_id = ?', [id]);
        await db_1.default.query('DELETE FROM h5p_contents WHERE id = ?', [id]);
        if (lessonId) {
            const [usage] = await db_1.default.query(`SELECT
           (SELECT COUNT(*) FROM lesson_contents WHERE lesson_id = ?) +
           (SELECT COUNT(*) FROM assessments WHERE lesson_id = ?) +
           (SELECT COUNT(*) FROM speaking_tests WHERE lesson_id = ?) AS usage_count`, [lessonId, lessonId, lessonId]);
            ...
            if (Number(usage[0]?.usage_count || 0) === 0) {
                await db_1.default.query('DELETE FROM lesson_progress WHERE lesson_id = ?', [lessonId]);
                await db_1.default.query('DELETE FROM lesson_bookmarks WHERE lesson_id = ?', [lessonId]).catch(() => { });
                await db_1.default.query('DELETE FROM lessons WHERE id = ?', [lessonId]);
            }
        }
...
exports.deleteH5P = deleteH5P;
// 6. Attach H5P Content directly into Lesson Syllabus
const attachH5PToLesson = async (req, res) => {
    const { id } = req.params;
    ...
    try {
        await ensureH5PTables();
        const [h5pRows] = await db_1.default.query('SELECT title, lesson_id, params_json FROM h5p_contents WHERE id = ?', [id]);
        if (h5pRows.length === 0) {
            return res.status(404).json({ error: 'Materi H5P tidak ditemukan' });
        }
        const h5p = h5pRows[0];
        ...
        if (targetLessonId) {
            const [lRows] = await db_1.default.query('SELECT lesson_type FROM lessons WHERE id = ?', [targetLessonId]);
            if (lRows.length === 0) {
            ...
            const [moduleRows] = await db_1.default.query(`SELECT m.id, cv.course_id
         FROM modules m
         JOIN course_versions cv ON cv.id = m.course_version_id
         WHERE m.id = ?`, [targetModuleId]);
            ...
            if (h5p.lesson_id) {
                const [existingLesson] = await db_1.default.query(`SELECT l.id,
             (SELECT COUNT(*) FROM lesson_contents lc
              WHERE lc.lesson_id = l.id AND lc.content_type <> 'H5P') AS regular_content_count
           FROM lessons l WHERE l.id = ?`, [h5p.lesson_id]);
                if (existingLesson.length > 0 && Number(existingLesson[0].regular_content_count || 0) === 0) {
                    targetLessonId = Number(h5p.lesson_id);
                    await db_1.default.query(`UPDATE lessons
             SET module_id = ?, lesson_type = 'READING', title = ?, slug = ?, duration_minutes = 15, xp_reward = 20, status = 'PUBLISHED'
             WHERE id = ?`, [targetModuleId, finalTitle, slugify(finalTitle), targetLessonId]);
                }
            }
            if (!targetLessonId) {
                const [maxOrderRows] = await db_1.default.query('SELECT COALESCE(MAX(lesson_order), 0) AS max_order FROM lessons WHERE module_id = ?', [targetModuleId]);
                const [lessonResult] = await db_1.default.query(`INSERT INTO lessons
           ...
           VALUES (?, 'READING', ?, ?, ?, 15, 0, 1, 70, 20, NULL, 'PUBLISHED')`, [targetModuleId, finalTitle, slugify(finalTitle), Number(maxOrderRows[0]?.max_order || 0) + 1]);
                targetLessonId = lessonResult.insertId;
            }
            if (h5p.lesson_id && Number(h5p.lesson_id) !== targetLessonId) {
                await db_1.default.query(`DELETE FROM lesson_contents
           WHERE lesson_id = ? AND content_type = 'H5P' AND attachments LIKE ?`, [h5p.lesson_id, `%"h5p_id":${Number(id)}}%`]);
            }
        }
        // 1. Update H5P record
        await db_1.default.query(`
...
exports.attachH5PToLesson = attachH5PToLesson;
// 7. Student Submit Attempt / Interaction
const submitH5PAttempt = async (req, res) => {
    const { id } = req.params;
    ...
    try {
        await ensureH5PTables();
        const [h5pRows] = await db_1.default.query('SELECT lesson_id FROM h5p_contents WHERE id = ?', [id]);
        ...
        try {
            ...
            const result = await (0, progress_1.updateProgressHelper)(connection, userId, lessonId, true, 100);
...
exports.submitH5PAttempt = submitH5PAttempt;
// 8. Upload Media File (Images, GIFs, Audio clips for H5P Elements)
const uploadH5PMedia = async (req, res) => {
    try {
        if (!req.file) {
...
exports.uploadH5PMedia = uploadH5PMedia;
