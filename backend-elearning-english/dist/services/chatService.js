...
exports.getOrCreateStudentRoom = getOrCreateStudentRoom;
exports.getStudentConversation = getStudentConversation;
exports.sendChatMessage = sendChatMessage;
exports.markChatAsRead = markChatAsRead;
exports.getAdminChatConversations = getAdminChatConversations;
const db_1 = __importDefault(require("../config/db"));
/**
 * Get or create a chat room for a specific student
 */
async function getOrCreateStudentRoom(studentId) {
    // 1. Verify student exists
    const [userRows] = await db_1.default.query(`SELECT id, full_name, email, avatar FROM users WHERE id = ?`, [studentId]);
    if (userRows.length === 0) {
        throw new Error(`Student with ID ${studentId} not found`);
    }
    const student = userRows[0];
    // 2. Check if room exists
    const [roomRows] = await db_1.default.query(`SELECT id, student_id, last_message, last_message_at, last_sender_id,
    ...
    // Create new room
    const [insertRes] = await db_1.default.query(`INSERT INTO chat_rooms (student_id, status) VALUES (?, 'OPEN')`, [studentId]);
 ...
 */
async function getStudentConversation(studentId, options = {}) {
    ...
    const [rows] = await db_1.default.query(`SELECT m.id, m.room_id, m.sender_id, m.sender_role, m.message, m.attachment_url,
...
async function sendChatMessage(params) {
    ...
    // 1. Insert message
    const [insertRes] = await db_1.default.query(`INSERT INTO chat_messages (room_id, sender_id, sender_role, message, attachment_url, is_read)
     VALUES (?, ?, ?, ?, ?, 0)`, [
        room.id,
        params.senderId,
        params.senderRole,
    ...
    await db_1.default.query(`UPDATE chat_rooms
     SET last_message = ?,
         last_message_at = NOW(),
         last_sender_id = ?,
    ...
    const [senderRows] = await db_1.default.query(`SELECT full_name, avatar FROM users WHERE id = ?`, [params.senderId]);
 ...
 */
async function markChatAsRead(params) {
    const room = await getOrCreateStudentRoom(params.studentId);
    ...
    const [updateRes] = await db_1.default.query(updateQuery, [room.id]);
    await db_1.default.query(resetRoomCounterQuery, [room.id]);
    return {
        roomId: room.id,
 ...
 */
async function getAdminChatConversations(options = {}) {
    ...
    const [summaryRows] = await db_1.default.query(`SELECT COUNT(r.id) as total_conversations,
    ...
    // Data query
    const [rows] = await db_1.default.query(`SELECT r.id, r.student_id, r.last_message, r.last_message_at, r.last_sender_id,
