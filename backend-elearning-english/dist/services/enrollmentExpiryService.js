...
exports.checkAndDispatchExpiryNotifications = checkAndDispatchExpiryNotifications;
exports.getStudentExpiringCourses = getStudentExpiringCourses;
exports.getAdminExpiringEnrollments = getAdminExpiringEnrollments;
exports.initEnrollmentExpiryScheduler = initEnrollmentExpiryScheduler;
 ...
 */
async function checkAndDispatchExpiryNotifications() {
    let notificationsSent = 0;
    let expiredUpdated = 0;
    let totalChecked = 0;
    try {
        // 1. Process ACTIVE enrollments that have passed their expiration date -> Mark as EXPIRED
        const [expiredEnrollments] = await db_1.default.query(`SELECT e.id, e.user_id, e.course_id, e.expired_at,
       ...
       WHERE e.status = 'ACTIVE'
        ...
        for (const item of expiredEnrollments) {
            ...
            await db_1.default.query(`UPDATE enrollments SET status = 'EXPIRED' WHERE id = ?`, [item.id]);
            expiredUpdated++;
            // Check deduplication tag in user_notifications
            const refTag = `[Ref: ENROLL-${item.id}-HEXPIRED]`;
            const [existingNotif] = await db_1.default.query(`SELECT id FROM user_notifications WHERE user_id = ? AND message LIKE ? LIMIT 1`, [item.user_id, `%${refTag}%`]);
            if (existingNotif.length === 0) {
                ...
                await db_1.default.query(`INSERT INTO user_notifications (user_id, title, message, is_read, created_at) VALUES (?, ?, ?, 0, NOW())`, [item.user_id, title, message]);
                notificationsSent++;
            }
        }
        // 2. Process ACTIVE enrollments expiring within 7 days
        const [expiringEnrollments] = await db_1.default.query(`SELECT e.id, e.user_id, e.course_id, e.expired_at,
              c.title as course_title,
              u.full_name, u.email,
 ...
 */
async function getStudentExpiringCourses(userId) {
    const [rows] = await db_1.default.query(`SELECT e.id as enrollment_id, e.course_id, e.enrolled_at, e.expired_at, e.access_days, e.status,
 ...
 */
async function getAdminExpiringEnrollments(options = {}) {
    ...
    // Count total query
    const [countResult] = await db_1.default.query(`SELECT COUNT(DISTINCT e.id) as total
     FROM enrollments e
     JOIN users u ON e.user_id = u.id
     JOIN courses c ON e.course_id = c.id
     ${whereClause}`, params);
    ...
    // Data query
    const [rows] = await db_1.default.query(`SELECT e.id, e.user_id, e.course_id, e.enrolled_at, e.expired_at, e.access_days, e.status,
 ...
 */
let expirySchedulerInterval = null;
function initEnrollmentExpiryScheduler() {
